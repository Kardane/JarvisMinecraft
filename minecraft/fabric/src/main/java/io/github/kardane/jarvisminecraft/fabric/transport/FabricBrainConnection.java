...
public final class FabricBrainConnection {
    ...
    private ProtocolMessage.Capability capability(String name) {
        return new ProtocolMessage.Capability(name, "Fabric", minecraftVersion);
    }
    ...
    private void handleToolRequest(BrainTransport source, ProtocolMessage message) {
        if (!bindings.matches(message) || !activeSession.test(message.requesterUuid(), message.sessionId())) {
            sendToolResult(
                ...
                ToolResult.error(
                    ...
                    "Tool request is not bound to an active Fabric operator request.",
                    false,
                    clock.instant(),
                    "Fabric"
                )
        ...
        if (runtime == null) {
            sendToolResult(
                ...
                ToolResult.error(
                    ErrorCode.CANCELLED,
                    "Fabric connection is no longer active.",
                    false,
                    clock.instant(),
                    "Fabric"
                )
    ...
    private final class Listener implements BrainTransport.Listener {
        ...
        public void onConnected() {
            FabricBrainConnection.this.onConnected(source);
        }

        @Override
        public void onMessage(String message) {
            FabricBrainConnection.this.onMessage(source, message);
        }
