...
public final class NeoForgeBrainConnection {
    ...
    public NeoForgeBrainConnection(
        String serverId,
    ...
    private ProtocolMessage.Capability capability(String name) {
        return new ProtocolMessage.Capability(name, "NeoForge", minecraftVersion);
    }
    ...
    private void handleToolRequest(BrainTransport source, ProtocolMessage message) {
        if (!bindings.matches(message) || !activeSession.test(message.requesterUuid(), message.sessionId())) {
            sendToolResult(
                ...
                ToolResult.error(
                    ...
                    "Tool request is not bound to an active NeoForge operator request.",
                    false,
                    clock.instant(),
                    "NeoForge"
                )
        ...
        if (runtime == null) {
            sendToolResult(
                ...
                ToolResult.error(
                    ErrorCode.CANCELLED,
                    "NeoForge connection is no longer active.",
                    false,
                    clock.instant(),
                    "NeoForge"
                )
        ...
        serverScheduler.submit(() -> runtime.execute(message)).whenComplete((result, failure) -> {
            ToolResult terminal = failure == null
                ...
                : ToolResult.error(
                    ErrorCode.INTERNAL,
                    "NeoForge Tool execution failed.",
                    false,
                    clock.instant(),
                    "NeoForge"
                );
    ...
    private final class Listener implements BrainTransport.Listener {
        ...
        public void onConnected() {
            NeoForgeBrainConnection.this.onConnected(source);
        }

        @Override
        public void onMessage(String message) {
            NeoForgeBrainConnection.this.onMessage(source, message);
        }
