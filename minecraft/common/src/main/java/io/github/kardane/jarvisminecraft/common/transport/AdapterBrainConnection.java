...
public final class AdapterBrainConnection {
    ...
    private void handleToolRequest(BrainTransport source, ProtocolMessage message) {
        if (!bindings.matches(message) || !activeSession.test(message.requesterUuid(), message.sessionId())) {
            sendToolResult(
                ...
                ToolResult.error(
                    ...
                    "Tool request is not bound to an active " + platformDisplayName + " operator request.",
                    false,
                    clock.instant(),
                    platformDisplayName
                )
        ...
        if (runtime == null) {
            sendToolResult(
                ...
                ToolResult.error(
                    ErrorCode.CANCELLED,
                    platformDisplayName + " connection is no longer active.",
                    false,
                    clock.instant(),
                    platformDisplayName
                )
        ...
        serverScheduler.submit(() -> runtime.execute(message)).whenComplete((result, failure) -> {
            ToolResult terminal = failure == null
                ...
                : ToolResult.error(
                    ErrorCode.INTERNAL,
                    platformDisplayName + " Tool execution failed.",
                    false,
                    clock.instant(),
                    platformDisplayName
                );
    ...
    private final class Listener implements BrainTransport.Listener {
        ...
        public void onConnected() {
            AdapterBrainConnection.this.onConnected(source);
        }

        @Override
        public void onMessage(String message) {
            AdapterBrainConnection.this.onMessage(source, message);
        }
